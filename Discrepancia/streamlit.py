...
# Exibe miniaturas no momento do upload
if rg_image:
    st.image(rg_image, caption="RG", width=150)
if selfie_image:
    st.image(selfie_image, caption="Selfie", width=150)
...
if rg_image and selfie_image:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            # Abre as imagens sem redimensionar
            image_rg = Image.open(rg_path).convert("RGB")
            image_selfie = Image.open(selfie_path).convert("RGB")
